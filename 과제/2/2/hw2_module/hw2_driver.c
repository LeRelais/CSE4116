...
static int driver_read(struct file *inode, char *gdata, size_t length, loff_t *off_what);
...
/*
	dirver_exit(void) : Discard physical mapping information between FPGA modules and 
						RAM. Unregister timer device.
*/
static void __exit driver_exit(void){
	printk("driver exit()\n");
	iounmap(fpga_dot);
...
/*
	driver_read(void) : Function for reading inputs from PUSH_SWITCH. When read() for device is called from
						application, driver_read() will be executed.
*/
...
static int driver_read(struct file *inode, char *gdata, size_t length, loff_t *off_what){
	...
	if (copy_to_user(gdata, &push_sw_value, length))
		return -EFAULT;
	// printk("\npush sw in kernel : %d\n", push_sw_value[0]);
	if(push_sw_value[0] == 80)
		return RESET_RECEIVED;	
	return 1;
}

/*
	driver_open(void) : When p[en() is called from
						application for device, driver_open() will be executed.
*/
...
/*
	driver_release(void) : callback for release() of driver
*/
...
/*
	driver_ioctl(void) : Function for ioctl handling. when ioctl() is called, 
						 driver will execute approriation option defined in switch logic. 
*/
static long driver_ioctl(struct file* file, unsigned int cmd, unsigned long arg) {
	...
	switch (cmd) {
		...
		case IOCTL_SET:
			...
			/* 
				Initialize FPGA modules with initial values
				
				FND : TIMER_INIT
				LCD : "20181625     TIMER_CNT"
				      "Name initial          "  
				DOT : none zero value of TIMER_INIT
				LED : LED number according to none zero value

			*/
			fpga_dot_init(timer_val);
			fpga_fnd_init(timer_init);
			fpga_lcd_init(lcd_str);
			fpga_led_init(timer_val); 
		...
		default: 
			/* print error message for undefined ioctl request */
...
/*
	timer_after_timer_cnt() : function that will be executed when timer2.expires  expires.
							  LCD will be updated based on the requirements.
...
/*
	timer_blink() : function that will be executed when timer.expires  expires.
					FPGA module will be updated based on the requirements.
...
/*
	fpga_dot_init() : Set DOT in respect to value passed as parameter.
*/
void fpga_dot_init(int value){
	int i = 0;
	unsigned short int _s_value;
...
/*
	fpga_dot_clear() : clear DOT module. 
*/
void fpga_dot_clear(){
	int i = 0;
	unsigned short int _s_value;
...
/*
	fpga_fnd_init() : Set FND in respect to str passed as parameter.
*/
...
/*
	fpga_lcd_init() : Set LCD in respect to str passed as parameter.
*/
...
/*
	fpga_led_init() : Set LED in respect to value passed as parameter.
*/
...
/*
	fpga_fnd_blink() : update FND per timer interval based on requirements from assignment specification. 
*/
void fpga_fnd_blink(){
	int i, idx;
...
/*
	fpga_led_blink() : update LED per timer interval based on requirements from assignment specification.
*/
void fpga_led_blink(){
	int i, idx;
...
/*
	fpga_dot_blink() : update DOT per timer interval based on requirements from assignment specification.
*/
void fpga_dot_blink(){
	int i, idx;
...
/*
	fpga_lcd_blink_after_timer() : update closing message shown in LCD per timer interval based on requirements from assignment specification. 
*/
void fpga_lcd_blink_after_timer(){
	char lcd_str[33];
	int i =0;
...
/*
	fpga_lcd_blink() : update LCD per timer interval based on requirements from assignment specification.
*/
void fpga_lcd_blink(){
	int i;
	...
	//update TIMER_CNT part of first line for LCD
	if(timer.cnt_tmp != 0){
	...
	//update second line of LCD. If it reaches 31th idx when direction is 2 (right), change direction to 0 (left).
	if(timer.dir == 2){
		timer.start_idx++;
